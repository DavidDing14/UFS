...
%DCT2逆变换????
idct2graypic=64*idct2(dct2graypic)/255;
subplot(3,3,5),imshow(idct2graypic),title('idct2之后的灰度图');

%1-D DCT变换
dctgraypic = double(zeros(rows, cols));
for i=1:rows
    dctgraypic(i,:) = (1/8)*dct(double(J(i,:)));
end
for i=1:cols
    dctgraypic(:,i) = (1/8)*dct(dctgraypic(:,i));
end
subplot(3,3,6);
imshow(dctgraypic);
title('1D DCT处理后图像');
...
%1-D IDCT变换
idctgraypic=64*idct2(dctgraypic)/255;
subplot(3,3,7);
imshow(idctgraypic);
title('1D IDCT还原图像');
...
for i=1:rows/8
    for j=1:cols/8
        dct2pic(i*8-7:i*8,j*8-7:j*8) = (1/64)*dct2(J(i*8-7:i*8,j*8-7:j*8));
    end
end
subplot(3,3,8);
imshow(dct2pic);
...
for i=1:rows/8
    for j=1:cols/8
        idct2pic(i*8-7:i*8,j*8-7:j*8) = 64*idct2(dct2pic(i*8-7:i*8,j*8-7:j*8))/255;
    end
end
subplot(3,3,9);
imshow(idct2pic);
